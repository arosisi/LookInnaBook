...
begin
	select sum(quantity) + NEW.quantity into newQuantity
	from cart_book, cart
	where cart_book.isbn = NEW.isbn AND
          cart.order_id = cart_book.order_id AND
          to_date(cart.date, 'Month DD YYYY') > now() - interval '1 month';
...
drop trigger refill_book on book;

create trigger refill_book after update of quantity
on book
for each row
when (
    OLD.quantity IS DISTINCT FROM NEW.quantity and
    NEW.quantity < OLD.threshold
)
